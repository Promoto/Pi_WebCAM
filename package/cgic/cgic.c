...
char *cgiUserAgent;
char *cgiReferrer;
e_cgi_web_protocol web_protocol;
...
#define IS_WEB_STD_PORT(a, b) ((a == CGIC_WEB_PROTOCOL_HTTPS && b == 443) ||	\
								(a == CGIC_WEB_PROTOCOL_HTTP && b == 80))
void cgiGetHostStr(char* buf){
	int port = atoi(cgiServerPort);
	char port_buf[10];
	sprintf(port_buf, ":%d", port);
	sprintf(buf, "%s://%s%s", 
				(web_protocol == CGIC_WEB_PROTOCOL_HTTPS)?"https":"http",
				cgiServerName,
				IS_WEB_STD_PORT(web_protocol, port)?"":port_buf);
	return;
}
...
int main(int argc, char *argv[]) {
	...
	if(!memcmp(cgiReferrer, "https", 5)){
		web_protocol = CGIC_WEB_PROTOCOL_HTTPS;
	}
	else{
		web_protocol = CGIC_WEB_PROTOCOL_HTTP;
	}
...
e_form_type form_type = CGIC_FORM_TYPE_NONE;

int cgiFormInit(e_form_type type){
	form_type = type;
	if(form_type == CGIC_FORM_TYPE_NORMAL){
		if (cgiFormEntries(&g_cgic_value_ptr) != cgiFormSuccess) {
			return -1;
		}
	}
	return 1;
}
cgiFilePtr form_value_fp;
e_cgi_form_status cgiFormValue(char* name, char* buf, int len, e_form_value_type type){
	int ret;
	char **arrayStep;
	if(form_type == CGIC_FORM_TYPE_NORMAL){
		if(type != CGIC_FORM_VALUE_TEXT)
			return CGI_FORM_VALUE_FAIL;
		for(arrayStep = g_cgic_value_ptr; *arrayStep; arrayStep++){
			if(!strcmp(*arrayStep, name)){
				ret = cgiFormString(*arrayStep, buf, len);
				if(ret != cgiFormSuccess){
					return CGI_FORM_VALUE_FAIL;
				}
				else{
					return CGI_FORM_VALUE_SUCCESS;
				}
			}
		}
	}
	else if(form_type == CGIC_FORM_TYPE_MULTIPART){
		if(type == CGIC_FORM_VALUE_FILE){
			if ((ret = cgiFormFileOpen(name, &form_value_fp, buf, len)) != cgiFormSuccess) {
		        return CGI_FORM_VALUE_FAIL;
		    }
		}
		else if(type == CGIC_FORM_VALUE_TEXT){
			cgiFormString(name, buf, len);
		}
	}
	return CGI_FORM_NAME_NOT_FOUND;
}

void cgiFormClose(void){
	if(form_type == CGIC_FORM_TYPE_MULTIPART){
		cgiFormFileClose(form_value_fp);
	}
}
...
cgiFormResultType cgiFormFileOpen(
	char *name, cgiFilePtr *cfpp, char* buf, int len)
{
	cgiFormEntry *e;
	cgiFilePtr cfp;
	e = cgiFormEntryFindFirst(name);
	...
	if(strlen(e->tfileName) > len){
		return cgiFormFileNameTooLong;
	}
	else{
		strcpy(buf, e->tfileName);
	}
	if (!cfp->in) {
		free(cfp);
		return cgiFormIO;
	...
	char *path, char *domain, int options)
{
	...
	*/
	...
	gt = gmtime(&then);
	fprintf(cgiOut, 
		"Set-Cookie: %s=%s; expires=%s, %02d-%s-%04d %02d:%02d:%02d GMT; path=%s%s%s%s\r\n",
		name, value, 
		//domain, 
		days[gt->tm_wday],
		gt->tm_mday,
...
cgiEnvironmentResultType cgiWriteEnvironment(char *filename) {
	...
	while (e) {
		...
		if (cgiFormFileOpen(e->attr, &fp, buf, 100) == cgiFormSuccess) {
			char buffer[1024];
			int got;
