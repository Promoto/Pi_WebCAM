...
/* The various CGI environment variables. Instead of using getenv(),
	...
	for debugging. */
typedef enum{
	CGI_FORM_VALUE_SUCCESS,
	CGI_FORM_VALUE_FAIL,
	CGI_FORM_NAME_NOT_FOUND
}e_cgi_form_status;

typedef enum{
	CGIC_FORM_TYPE_NONE,
	CGIC_FORM_TYPE_NORMAL,
	CGIC_FORM_TYPE_MULTIPART
}e_form_type;

typedef enum{
	CGIC_FORM_VALUE_NONE,
	CGIC_FORM_VALUE_TEXT,
	CGIC_FORM_VALUE_FILE
}e_form_value_type;

typedef enum{
	CGIC_WEB_PROTOCOL_HTTP,
	CGIC_WEB_PROTOCOL_HTTPS
}e_cgi_web_protocol;

extern char **g_cgic_value_ptr;
extern int cgiFormInit(e_form_type type);
extern void cgiGetHostStr(char* buf);
extern e_cgi_form_status cgiFormValue(char* name, char* buf, int len, e_form_value_type type);
extern void cgiFormClose(void);
...
extern e_cgi_web_protocol web_protocol;
...
typedef enum {
	...
	cgiFormIO,
	cgiFormEOF,
	cgiFormFileNameTooLong
} cgiFormResultType;

#define cgicPrintHtmlHeader do{											\
	fprintf(cgiOut, "Pragma: No-cache\r\n");							\
	fprintf(cgiOut, "Cache-Control: no-cache,no-store,max-age=0\r\n");	\
	fprintf(cgiOut, "X-Frame-Options: SAMEORIGIN\r\n\r\n\r\n");			\
}while(0)

#define cgiPrintCgiHeaderStart do{										\
	fprintf(cgiOut, "Pragma: No-cache\r\n");							\
	fprintf(cgiOut, "Cache-Control: no-cache,no-store,max-age=0\r\n");	\
}while(0)

#define cgiPrintCgiHeaderEnd do{						\
	cgiHeaderContentType("text/xml;charset=UTF-8\r\n\r\n");	\
}while(0)

#define cgiPrintCgiHeaderAdd(a) do{	\
	fprintf(cgiOut, a);				\
	fprintf(cgiOut, "\r\n");		\
}while(0);
